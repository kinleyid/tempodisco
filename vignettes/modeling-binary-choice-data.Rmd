...
For binary choice data not explicitly designed to titrate out indifference points (as in an adjusting amount procedure), there are a few analysis options. Historically common methods include the scoring method designed for the Monetary Choice Questionnaire [(Kirby, 1999)](https://doi.org/10.1037//0096-3445.128.1.78):
...
This approach turns out to be possible for other discount functions as well [(Kinley et al., 2024)](https://doi.org/10.31234/osf.io/y2fdh). We can test all of them and select the best one according to the Bayesian Information Criterion as follows:
...
To fit an even wider range of discount functions, we can fit a nonlinear regression model by calling `td_bcm`. A full list of the available discount functions can be found in the documentation for `td_bcm`.

```{r}
mod <- td_bcm(td_bc_single_ptpt, discount_function = 'all')
print(mod)
```
...
```{r}
# Probit choice rule:
mod <- td_bcm(td_bc_single_ptpt, discount_function = 'exponential', choice_rule = 'probit')
# Power choice rule:
mod <- td_bcm(td_bc_single_ptpt, discount_function = 'exponential', choice_rule = 'power')
```
...
mod <- td_bcm(df, discount_function = 'exponential', fit_err_rate = T)
...
```{r}
mod <- td_bcm(df, discount_function = 'exponential', fixed_ends = T)
plot(mod, type = 'endpoints', verbose = F)
```

...
As we can see, parameters are estimated not only for the discount function ($k$), but also for the DDM ($v$, $\beta$, $\alpha$ and $\tau$). These can be extracted using `coef()` to collect for group-level analysis (see the vignette ["Analyzing data from multiple participants"](analyzing-study.html)). As with other model classes (`td_bcnm`, `td_bclm`), we can plot the resulting discount function:

...
#' @param mod A temporal discounting model. See `tdbcm`.
...
#' @export
predict.tdbcm <- function(mod, newdata = NULL, type = c('link', 'response', 'indiff')) {
  if (is.null(newdata)) {
    newdata <- mod$data
  }
